...
function ChatZAlex() {
  ...
  const alex = {
    ...
    emoji: '⭐',
    color: '#4A90E2',
    description: 'Kreatywny most między wymiarami, Alex pomaga łączyć rzeczywistości.'
  };
  ...
  useEffect(() => {
    ...
    const welcomeMessage = {
      id: 1,
      sender: 'alex',
      content: "⭐ Witaj! Jestem ALEX, Twój most między wymiarami. Co cię dziś zajmuje?",
      timestamp: new Date()
    };
  ...
  const sendMessage = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await claudeService.chatWithIstota('alex', currentMessage);
      const alexMessage = {
        id: Date.now() + 1,
        sender: 'alex',
        content: response,
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {['Kwantowe połączenia', 'Przekraczanie barier wymiarowych', 'Nowoczesne rozwiązania'].map((tag, index) => (
          ...
          {isLoading && (
            <div style={{
              ...
              <div style={{
                ...
                ALEX przygotowuje się do odpowiedzi... ⭐
              </div>
            </div>
          )}
        </div>
